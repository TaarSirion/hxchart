...
class Chart {
	public static function draw(x_points:Array<Float>, y_points:Array<Float>, ?x:Float = 0, ?y:Float = 0, ?width:Float, ?height:Float) {
		...
		drawPoints(canvas.componentGraphics, x_points, y_points, width, height, margin, axis_info, min_x, max_x, min_y, max_y);
	...
	private static function drawAxis(graphics:ComponentGraphics, screen:Screen, margin:Float, height:Float, width:Float, min_x:Float, max_x:Float,
		...
		trace("X axis info");
		trace(y_tick_info.min, y_tick_info.num, y_tick_info.prec, y_tick_info.step);
		var tick_margin = 10;
		...
		trace(margin_bottom, y_tick_boundary, y_tick_info.pos_ratio);
		...
		var y_width = margin + margin_left + 2 * tick_margin;
	...
	private static function drawPoints(graphhics:ComponentGraphics, x_points:Array<Float>, y_points:Array<Float>, width:Float, height:Float, margin:Float,
			axis_info:AxisInfo, min_x:Float, max_x:Float, min_y:Float, max_y:Float) {
		graphhics.circle(axis_info.x_ticks[0].position, axis_info.y_ticks[0].position, 3);
		graphhics.circle(axis_info.x_ticks[0].position, axis_info.y_ticks[axis_info.y_ticks.length - 1].position, 3);
		graphhics.circle(axis_info.x_ticks[axis_info.x_ticks.length - 1].position, axis_info.y_ticks[0].position, 3);
		...
		var y_dist = calcAxisDists(y_coord_min, y_coord_max, axis_info.y_tick_info.pos_ratio);
		trace("Zeroes", axis_info.x_tick_info.zero, axis_info.y_tick_info.zero, axis_info.x_ticks[axis_info.x_tick_info.zero]);
		trace("Dists", x_dist.pos_dist, x_dist.neg_dist, axis_info.x_tick_info.pos_ratio);
		trace("Dists y", y_dist.pos_dist, y_dist.neg_dist, axis_info.y_tick_info.pos_ratio, y_coord_min, y_coord_max);
